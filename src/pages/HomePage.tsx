...
import { useAppDispatch, useAppSelector } from "@/app/store/hooks";
import { getTenant } from "@/features/tenant/tenantThunk";
import { getSubscription, toggleSubscriptionStatus } from "@/features/subscription/subscriptionThunk";
import { Link } from "react-router-dom";
...
const HomePage = () => {
  ...
  const { name, isBlocked, isDeleted, loading: tenantLoading } = useAppSelector((state) => state.tenant);
  const { plan, status, endDate, loading: subLoading, toggling, id } = useAppSelector((state) => state.subscription);
        ...
        <div className="grid grid-cols-2 gap-6">
          ...
          <div className="bg-white p-6 rounded-xl shadow-sm">
            <div className="flex justify-between items-center mb-4">
              ...
              <span className={`text-xs px-2 py-1 rounded ${status === 'ACTIVE' ? 'bg-green-100 text-green-600' : 'bg-orange-100 text-orange-600'}`}>
                {status || 'INACTIVE'}
              </span>
            </div>

            <div className="text-sm text-gray-500 space-y-2">
              <p>Plan: {plan ? `${plan.charAt(0) + plan.slice(1).toLowerCase()} Subscription` : 'No Active Plan'}</p>
              <p>Billing: {plan === 'PRO' ? '₹4,999 / month' : plan === 'FREE' ? '₹0 / month' : 'Contact Sales'}</p>
              <p>Next Billing: {endDate ? new Date(endDate).toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' }) : 'N/A'}</p>
            </div>
