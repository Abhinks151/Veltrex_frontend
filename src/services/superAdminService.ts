...
export const superAdminService = {
  getAllTenants: async (params?: { page?: number; limit?: number; search?: string; status?: string }) => {
  ...
  toggleTenantBlock: async (id: string) => {
    return axiosInstance.patch(API_ENDPOINTS.SUPER_ADMIN.TOGGLE_TENANT_BLOCK(id));
  },

  updateTenantName: async (id: string, name: string) => {
    return axiosInstance.patch(API_ENDPOINTS.SUPER_ADMIN.UPDATE_TENANT_NAME(id), { name });
  },
};
