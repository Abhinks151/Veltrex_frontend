...
interface NcProgramFormProps {
  onSubmit: (data: FormData) => void;
  loading?: boolean;
}

const NcProgramForm: React.FC<NcProgramFormProps> = ({
  onSubmit,
  loading = false,
}) => {
  ...
  const {
    ...
    setValue,
    formState: { errors },
  } = useForm<CreateNcProgramFormData>({
    resolver: zodResolver(
      createNcProgramSchema,
    ) as Resolver<CreateNcProgramFormData>,
  ...
  const handleFormSubmit = (data: CreateNcProgramFormData) => {
  ...
  return (
    <form onSubmit={handleSubmit(handleFormSubmit)} className="space-y-4">
      ...
      <div className="space-y-1">
        ...
        <Input {...register('name')} placeholder="e.g. Fan Cover" />
        {errors.name && (
