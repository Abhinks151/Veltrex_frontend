...
export const deleteNcProgram = createAsyncThunk<
  ApiResponse<NcProgram>,
  string,
  { rejectValue: string }
>('ncProgram/delete', async (programId, { rejectWithValue }) => {
  try {
    const response = await ncProgramService.delete(programId);
    return response.data;
  } catch (error) {
    if (axios.isAxiosError(error)) {
      return rejectWithValue(
        error.response?.data.message || 'Failed to delete NC program',
      );
    }
    return rejectWithValue(
      FRONTEND_MESSAGE_CONSTANTS.ERROR.SOMETHING_WENT_WRONG,
