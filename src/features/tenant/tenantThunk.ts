...
import type { TenantRequest, TenantResponse, TenantUpdateRequest } from "./types";
import { tenantService } from "@/services/tenantService";
import { FRONTEND_MESSAGE_CONSTANTS } from '../../shared/constants/messageConstants';


export const tenant = createAsyncThunk<ApiResponse<TenantResponse>, TenantRequest, { rejectValue: string }>(
  "tenant/create",
  async (data: TenantRequest, { rejectWithValue }) => {
    try {
      ...
      if (axios.isAxiosError(error)) {
        // console.log(error)
        return rejectWithValue(error.response?.data.message || FRONTEND_MESSAGE_CONSTANTS.ERROR.TENANT_CREATION_FAILED);
      }
...
export const updateTenant = createAsyncThunk<ApiResponse<TenantResponse>, TenantUpdateRequest, { rejectValue: string }>(
  "tenant/update",
  async (data: TenantUpdateRequest, { rejectWithValue }) => {
    try {
      ...
      if (axios.isAxiosError(error)) {
        return rejectWithValue(error.response?.data.message || "Tenant update failed");
      }
      return rejectWithValue(FRONTEND_MESSAGE_CONSTANTS.ERROR.SOMETHING_WENT_WRONG);
    }
  }
